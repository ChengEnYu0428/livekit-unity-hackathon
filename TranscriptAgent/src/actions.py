...
完全沒有期限時 deadline 填「未指定」、deadline_date 填空字串。不要自己創造期限。
...
message 用一兩句繁體中文說明建立或修改了什麼；沒有要執行的工作就說明沒有新增待辦。"""
...
class TaskBoard:
    ...
    def find_duplicate(self, title: str, owner: str, deadline_date: str) -> dict | None:
        key = (normalize(title), normalize(owner), deadline_date)
        return next((t for t in self.tasks if (normalize(t["title"]), normalize(t["owner"]),
                     t["deadline_date"]) == key), None)
    ...
    def apply(self, operations) -> list[dict]:
        ...
        for op in operations[:MAX_OPERATIONS]:
            ...
            if not valid_date(deadline_date):
                deadline_date = ""
            if kind == "create":
                actions.extend(self.create(title, owner or "未指定", deadline or "未指定", deadline_date))
            elif kind == "update":
                actions.extend(self.update(clean_text(op.get("task_id"), 20), title, owner,
                                           deadline, deadline_date))
        return actions

    def create(self, title, owner, deadline, deadline_date) -> list[dict]:
        if not title:
            return []
        existing = self.find_duplicate(title, owner, deadline_date)
        if existing:
        ...
        task = {"id": uuid.uuid4().hex[:8], "title": title, "owner": owner, "deadline": deadline,
                "deadline_date": deadline_date, "created_at": now, "updated_at": now}
        self.tasks.append(task)
        actions = [{"type": "create_task", "status": "success", "task": dict(task)}]
        return actions + self.calendar_change(None, task)

    def update(self, task_id, title, owner, deadline, deadline_date) -> list[dict]:
        ...
        if owner:
            task["owner"] = owner
        if deadline:
            # A new spoken deadline replaces the date too; "未指定" clears it.
...
def weekday_of(value: str) -> str:
    return WEEKDAYS[date.fromisoformat(value).weekday()] if valid_date(value) else ""


def tasks_display_text(result: dict) -> str:
    tasks, events = result["tasks"], result["calendar_events"]
    lines = [f"待辦事項（{len(tasks)}）"]
    for t in tasks:
        due = t["deadline"] + (f"（{t['deadline_date']}）" if t["deadline_date"] else "")
        lines.append(f"• {t['title']}｜負責人：{t['owner']}｜期限：{due}")
    if not tasks:
        lines.append("目前沒有待辦事項。")
    lines += ["", "行事曆"]
    for e in events:
        lines.append(f"{e['date']}（{weekday_of(e['date'])}）  {e['title']} — {e['owner']}")
    if not events:
        lines.append("沒有排定日期的待辦。")
    labels = {("create_task", "success"): "新增", ("create_task", "already_exists"): "已存在，未重複建立",
              ("create_task", "limit_reached"): "待辦已達上限，未建立",
              ("update_task", "success"): "修改", ("update_task", "not_found"): "找不到要修改的待辦",
              ("calendar_created", "success"): "加入行事曆", ("calendar_updated", "success"): "更新行事曆",
              ("calendar_removed", "success"): "移出行事曆"}
